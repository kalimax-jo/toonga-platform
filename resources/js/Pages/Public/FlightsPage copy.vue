...
<div v-if="showBookingModal" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
        ...
        <div class="bg-blue-50 border border-blue-200 rounded-lg p-4 mb-6">
          ...
          <div class="flex items-center gap-4 mb-3">
            ...
            <div class="ml-auto text-right">
              ...
              <p class="text-lg font-semibold text-green-700">{{ getConvertedPrice(selectedFlight.price) }}</p>
              ...
              <div class="mt-1 p-2 bg-green-50 rounded border border-green-200">
                <p class="text-sm font-medium text-green-800">Total: {{ calculateTotal() }}</p>
                <p class="text-xs text-green-600">{{ travelers }} × {{ getConvertedPrice(selectedFlight.price) }}</p>
              </div>
            </div>
          </div>
  ...
  <div class="flex justify-between">
    <span>Flight (EUR)</span>
    <span>{{ selectedFlight?.price || '€0' }}</span>
  </div>
  <div class="flex justify-between">
    <span>Flight (RWF)</span>
    <span>{{ selectedFlight ? getConvertedPrice(selectedFlight.price) : 'Rwf0' }}</span>
  </div>
  <div class="flex justify-between">
    <span>Travelers</span>
    <span>× {{ travelers }}</span>
  ...
  <div class="flex justify-between text-xs text-gray-500">
    ...
    <span>1 EUR = Rwf{{ exchangeRates?.RWF?.toLocaleString() || '1,617' }}</span>
  </div>
</div>
          </div>
...
const calculateTotal = () => {
  if (!selectedFlight.value?.price) return 'Rwf0'
  
  const match = selectedFlight.value.price.match(/[\d,]+\.?\d*/)
  if (!match) return 'Rwf0'
  
  const eurPrice = parseFloat(match[0].replace(/,/g, ''))
  const rwfRate = exchangeRates.value?.RWF || 1617
  const total = (eurPrice * rwfRate) * travelers.value
  
  return `Rwf${Math.round(total).toLocaleString()}`
}
...
const getConvertedPrice = (priceString) => {
  if (!priceString) return 'Rwf0.00'
  
  const match = priceString.match(/[\d,]+\.?\d*/)
  if (!match) return 'Rwf0.00'
  
  const eurPrice = parseFloat(match[0].replace(/,/g, ''))
  const rwfRate = exchangeRates.value?.RWF || 1615.782707242
  const convertedPrice = eurPrice * rwfRate  // Keep full precision
  
  return formatCurrency(convertedPrice, 'RWF')
}

const formatCurrency = (amount, currency) => {
  const symbols = { EUR: '€', USD: '$', RWF: 'Rwf' }
  
  if (currency === 'RWF') {
    // Keep 2 decimal places for RWF to maintain precision
